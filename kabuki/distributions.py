    ...
    class new_class(Stochastic):
        ...
        def __init__(self, *args, **kwds):
            ...
            if not mv:
                ...
                if (
                    init_val_shape is not None
                    and shape is not None
                    and init_val_shape != shape
                ):
                    shape_error()
            ...
            if arg_dict_out.pop("debug"):
                ...
                Stochastic.__init__(
                    self, logp=logp, random=random, dtype=dtype, **arg_dict_out
                )
...
def scipy_stochastic(scipy_dist, **kwargs):
    ...
    if isinstance(scipy_dist, sc_dst.rv_continuous):
        ...
        defaults = [None] * (len(parent_names) - 2) + [0.0, 1.0]
    ...
    docstr = (
        ...
        + "(name, "
        + ", ".join(parent_names)
        + ", value=None, shape=None, trace=True, rseed=True, doc=None)\n\n"
    )
    docstr += (
...
reporting the bug.
    ...
    new_class = new_dist_class(
        dtype, name, parent_names, parents_default, docstr, logp, random, True, None
    )
