...
class TestStepMethods(unittest.TestCase):
    ...
    def assert_results(
        self, node, true_value, true_mean, true_std=None, mean_tol=0.1, std_tol=0.2
    ):
        ...
        if true_std is not None:
            np.testing.assert_allclose(
                node.stats()["standard deviation"], true_std, rtol=std_tol
            )
    ...
    def normal_normal(
        ...
        tau_0 = sigma_0**-2
        mu_0 = 0.0
        nodes, size, x_values = self.create_nodes_for_normal_normal(
            add_shift, tau_0, mu_0, sigma_beta, sigma_y, true_mu, n_subjs, avg_samples
        )
        mu = nodes["mu"]
        ...
        true_std = 1.0 / np.sqrt(tau_prime)

        self.assert_results(mu, true_mu, mu_prime, true_std, mean_tol=0.1, std_tol=0.1)

        return mm, mu_prime, true_std

    def create_nodes_for_normal_normal(
        self, add_shift, tau_0, mu_0, sigma_beta, sigma_y, true_mu, n_subjs, avg_samples
    ):
        ...
        for i in range(n_subjs):
            ...
            if add_shift:
                ...
                y = pm.Normal(
                    "y%d" % i, mu + x, sigma_y**-2, value=value, observed=True
                )
                nodes["x%d" % i] = x
                b.append(x)
            else:
    ...
    def create_nodes_for_PriorNormalstd(self, n_subjs, sigma_0, mu_0, prior, **kwargs):
        ...
        if prior is pm.Uniform:
            sigma = pm.Uniform("sigma", self.uniform_lb, self.uniform_ub, value=1.0)
        ...
        for i_cond in range(n_conds):
            for i in range(n_subjs):
                ...
                x = pm.Normal(
                    "x%d" % i, mu_0[i_cond], sigma**-2, value=x_value, observed=True
                )
    ...
    def uniform_normalstd(self, sigma_0, mu_0, n_subjs, seed, use_metropolis):
        ...
        nodes, x_values = self.create_nodes_for_PriorNormalstd(
            n_subjs, sigma_0, mu_0, prior=pm.Uniform
        )
        sigma = nodes["sigma"]
        mm = pm.MCMC(nodes)
        ...
        # calc the new distrbution
        alpha = (n_subjs - 1) / 2.0
        beta = sum([(x - mu_0) ** 2 for x in x_values]) / 2.0
    ...
    def uniform_normalstd_multiple_conds_with_shared_sigma(
        self, sigma_0, mu_0, n_subjs, seed, use_metropolis
    ):
        """test estimation of Normal distribution std with uniform prior
        sigma_0 - the value of the std noe
        mu_0 - the value of the mu node
        ...
        # calc the new distrbution
        alpha = (n_subjs * n_conds - 1) / 2.0
        beta = 0
        for i_cond in range(n_conds):
            cur_x_values = x_values[i_cond * n_subjs : (i_cond + 1) * n_subjs]
            beta += sum([(x - mu_0[i_cond]) ** 2 for x in cur_x_values]) / 2.0
        true_mean = math.gamma(alpha - 0.5) / math.gamma(alpha) * np.sqrt(beta)
        anal_var = beta / (alpha - 1) - true_mean**2
        true_std = np.sqrt(anal_var)
    ...
    def test_uniform_normalstd_numerical_solution(self):
        ...
        the mean and std"""
        self.uniform_normalstd(
            sigma_0=0.5, mu_0=0, n_subjs=8, seed=1, use_metropolis=True
        )
        self.uniform_normalstd(
            sigma_0=1.5, mu_0=-100, n_subjs=4, seed=2, use_metropolis=True
        )
        self.uniform_normalstd(
            sigma_0=2.5, mu_0=2, n_subjs=5, seed=3, use_metropolis=True
        )
        self.uniform_normalstd(
            sigma_0=3.5, mu_0=-4, n_subjs=7, seed=4, use_metropolis=True
        )
    ...
    def test_UniformNormalstd_step_method(self):
        ...
        self.uniform_normalstd(
            sigma_0=0.5, mu_0=0, n_subjs=8, seed=1, use_metropolis=False
        )
        self.uniform_normalstd(
            sigma_0=1.5, mu_0=-100, n_subjs=4, seed=2, use_metropolis=False
        )
        self.uniform_normalstd(
            sigma_0=2.5, mu_0=2, n_subjs=5, seed=3, use_metropolis=False
        )
        self.uniform_normalstd(
            sigma_0=3.5, mu_0=-4, n_subjs=7, seed=4, use_metropolis=False
        )
        self.uniform_normalstd(
            sigma_0=4.5, mu_0=10, n_subjs=4, seed=5, use_metropolis=False
        )

    def test_uniform_normalstd_with_multiple_condition_numerical_solution(self):
        """test uniform_normalstd with Metropolis to evaluate the numerical solution of
        the mean and std"""
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=0.5, mu_0=(0, 2, 10), n_subjs=8, seed=1, use_metropolis=True
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=3.5, mu_0=(-100, 3), n_subjs=4, seed=2, use_metropolis=True
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=2.5, mu_0=(1, 2), n_subjs=5, seed=3, use_metropolis=True
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=0.5, mu_0=(-4, -3, 2, 1, 0), n_subjs=7, seed=4, use_metropolis=True
        )

    def test_UniformNormalstd_step_method_with_multiple_condition(self):
        """test UniformPriorNormalstd step method"""
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=0.5, mu_0=(0, 2, 10), n_subjs=8, seed=1, use_metropolis=False
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=3.5, mu_0=(-100, 3), n_subjs=4, seed=2, use_metropolis=False
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=2.5, mu_0=(1, 2), n_subjs=5, seed=3, use_metropolis=False
        )
        self.uniform_normalstd_multiple_conds_with_shared_sigma(
            sigma_0=0.5, mu_0=[-4], n_subjs=7, seed=4, use_metropolis=False
        )
    ...
    def half_cauchy_normal_std(
        self, sigma_0=1.0, mu_0=0.0, S=10, n_subjs=8, seed=1, use_metropolis=False
    ):
        ...
        np.random.seed(seed)
        nodes, x_values = self.create_nodes_for_PriorNormalstd(
            n_subjs, sigma_0, mu_0, prior=kabuki.utils.HalfCauchy, S=S, value=1
        )
        sigma = nodes["sigma"]
        ...
        def defective_posterior(x, n=n_subjs, beta=beta, S=S):
            gammapdf = (x**2) ** (-n / 2.0) * np.exp(-beta / (x**2))
            cauchy = S / (x**2 + S**2)
    ...
    def half_cauchy_bundle(self, use_metropolis):
        self.half_cauchy_normal_std(
            sigma_0=4.5, mu_0=0, n_subjs=8, seed=1, S=5, use_metropolis=use_metropolis
        )
        self.half_cauchy_normal_std(
            sigma_0=0.5,
            mu_0=-100,
        ...
        self.half_cauchy_normal_std(
            sigma_0=5.5, mu_0=2, n_subjs=5, seed=3, S=3, use_metropolis=use_metropolis
        )
        self.half_cauchy_normal_std(
            sigma_0=1.5, mu_0=-4, n_subjs=7, seed=4, S=10, use_metropolis=use_metropolis
        )
        self.half_cauchy_normal_std(
            sigma_0=4.5, mu_0=10, n_subjs=4, seed=5, S=15, use_metropolis=use_metropolis
        )
    ...
    def run_SPXcentered(self, sigma_x, n_subjs, size, mu_value, mu_step_method, seed):
        ...
        iter = 10000  # 100000
        burnin = 5000  # 90000
        nodes, t_values = self.create_hierarchical_model(
            sigma_x=sigma_x, n_subjs=n_subjs, size=size, mu_value=mu_value, seed=seed
        )
        mcmc = pm.MCMC(nodes)
        [mcmc.use_step_method(mu_step_method, node) for node in nodes["mu"]]

        # init mcmc with SPX step method
        nodes_spx, t_values = self.create_hierarchical_model(
            sigma_x=sigma_x, n_subjs=n_subjs, size=size, mu_value=mu_value, seed=seed
        )
        mcmc_spx = pm.MCMC(nodes_spx)
        mcmc_spx.use_step_method(
            kabuki.steps.SPXcentered,
            loc=nodes_spx["mu"],
        ...
        nodes_vpx, t_values = self.create_hierarchical_model(
            sigma_x=sigma_x,
            n_subjs=n_subjs,
            size=size,
            mu_value=mu_value,
        ...
        while i_try < max_tries:
            ...
            stats = dict(
                [("mu%d spx" % x, mcmc_spx.mu[x].stats()) for x in range(n_conds)]
            )
            ...
            stats.update(
                dict([("mu%d vpx" % x, mcmc_vpx.mu[x].stats()) for x in range(n_conds)])
            )
            ...
            stats.update(
                dict([("mu%d basic" % x, mcmc.mu[x].stats()) for x in range(n_conds)])
            )
    ...
    def test_SPX(self):
        ...
        print("*************** Test 1 ***************")
        self.run_SPXcentered(
            sigma_x=1,
            n_subjs=5,
            size=100,
            mu_value=4,
            mu_step_method=kabuki.steps.kNormalNormal,
            seed=1,
        )
        print("*************** Test 2 ***************")
        self.run_SPXcentered(
    ...
    def create_hierarchical_model(
        self, sigma_x=1, n_subjs=5, size=100, mu_value=4, seed=1, vec=False
    ):
        """
        ...
        for i_cond in range(n_conds):
            ...
            # init mu and sigma
            mu[i_cond] = pm.Normal("mu%d" % i_cond, 0, 100.0**-2, value=0)

            # create subj_nodes (x + y)
            if vec:
                subj_nodes[i_cond] = pm.Normal(
                    "x%d" % (i_cond), mu[i_cond], tau, size=n_subjs
                )
                data_nodes[i_cond] = MN(
                    "y%d" % (i_cond),
                    vec_mu=subj_nodes[i_cond],
                    tau=1,
                    value=value,
                    observed=True,
                )
                ...
                for i_subj in range(n_subjs):
                    ...
                    subj_nodes[i_cond][i_subj] = pm.Normal(
                        "x%d_%d" % (i_cond, i_subj), mu[i_cond], tau
                    )
                    data_nodes[i_cond][i_subj] = pm.Normal(
                        "y%d_%d" % (i_cond, i_subj),
    ...
    def run_SliceStep(
        self, sigma_x, n_subjs, size, mu_value, seed, left=None, max_tries=5
    ):
        # init basic  mcmc
        if np.isscalar(mu_value):
            n_conds = 1
        ...
        nodes_s, t_values = self.create_hierarchical_model(
            sigma_x=sigma_x, n_subjs=n_subjs, size=size, mu_value=mu_value, seed=seed
        )
        ...
        mcmc_s.use_step_method(
            kabuki.steps.SliceStep, nodes_s["sigma"], width=3, left=left
        )
        ...
        while i_try < max_tries:
            ...
            stats.update(
                dict([("mu%d S" % x, mcmc_s.mu[x].stats()) for x in range(n_conds)])
            )
    ...
    def test_SliceStep(self):
        ...
        print("*************** Test 2 ***************")
        self.run_SliceStep(
            sigma_x=1, n_subjs=5, size=10, mu_value=list(range(10)), seed=1
        )
